...
def train_fit_run(train_count,label_list,x_train,y_train,x_test,y_test):
    ...
    for ix,xnum in enumerate(randomlist):
        ...
        plt.imshow(x_train[xnum])
        plt.title(label_list[np.argmax(y_train[xnum])])
    ...
    plt.title("ACCURACY")
    plt.subplot(1,2,2)
    plt.plot(fithist.history["loss"], label="Train Loss")
    plt.plot(fithist.history["val_loss"], label="Valid Loss")
    plt.legend()
    plt.title("LOSS")
    plt.show()
